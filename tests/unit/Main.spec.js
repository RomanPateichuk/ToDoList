...
import Task from "../../src/components/molecules/TaskWrapper.vue";
...
describe("Тестирование компонента Main:", () => {
  ...
  test("вызов события callAddTask из дочернего компонента", () => {
    expect(
      wrapper.findComponent(AddTask).vm.$emit("callAddTask", "value")
    ).toBeTruthy();
  });
  ...
  test("Подключение компонента 'TaskWrapper'", () => {
    expect(wrapper.findAllComponents(Task).length).toEqual(7);
  });

  // wrapper.emitted()  проверка updateTasks
});
